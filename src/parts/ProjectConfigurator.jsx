    ...
    <div className="space-y-24 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="space-y-8">
        <div className="space-y-2">
          ...
          <h3 className="text-3xl font-black text-black">Enter your stair details</h3>
          <p className="text-gray-400 text-sm font-medium">Note: Pricing is for straight stairs only. If you have curved stairs, the final price may be adjusted.</p>
        </div>

        <div className="space-y-6">
          {[
            { label: "Number of steps *", key: "steps", placeholder: "Enter number of steps" },
            ...
            <div key={field.key} className="space-y-2">
              ...
              <input 
                ...
                onChange={(e) => setStairData({...stairData, [field.key]: e.target.value})}
              />
            </div>
          ))}
          <p className="text-[10px] text-gray-400 font-medium italic">Exact numbers help us provide a more accurate estimate.</p>
        </div>
      </div>
      ...
      <div className="space-y-8">
        <div className="space-y-2">
          ...
          <h3 className="text-3xl font-black text-black">Your stair material</h3>
          <p className="text-gray-400 text-sm font-medium">For stairs, we use premium LVP flooring for optimal durability.</p>
        </div>
        ...
        <div className="p-8 rounded-3xl border-2 border-[#C9A961] bg-[#C9A961]/5 flex gap-6 relative group cursor-pointer transition-all">
          ...
          <div className="space-y-2">
            <h4 className="font-black text-black text-xl tracking-tight">LVP 5mm or thicker</h4>
            <p className="text-[#C9A961] font-bold text-sm">Professional Grade Stair Flooring</p>
            <p className="text-xs text-gray-500 font-medium leading-relaxed max-w-lg">
              Premium Luxury Vinyl Plank with exceptional durability and core SPC strength. High quality stair finish that's durable, stylish, and easy to maintain.
            </p>
            <p className="pt-2 text-[10px] text-gray-400 font-bold uppercase tracking-widest">5mm+ core thickness • Premium wear layer</p>
          </div>
        </div>
      </div>
          ...
          {[
            ...
            >
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${
                stairData.timeline === opt ? "border-[#C9A961]" : "border-gray-200"
              }`}>
                {stairData.timeline === opt && <div className="w-2 h-2 rounded-full bg-[#C9A961]"></div>}
              </div>
              <span className={`font-bold ${stairData.timeline === opt ? "text-black" : "text-gray-400"}`}>{opt}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 lg:px-12 max-w-7xl relative z-10">
        <div className="flex flex-col lg:flex-row gap-16 items-start">
          ...
          <div className="flex-1 space-y-24">
            ...
            <div className="space-y-8">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-[#C9A961] text-black flex items-center justify-center font-black shadow-lg">1</div>
                <h3 className="text-2xl font-bold text-black uppercase tracking-wide">Project Type</h3>
              </div>
              ...
              <div className="grid sm:grid-cols-3 gap-5">
                {options.map((option) => (
                  ...
                  >
                    <div className={`p-3 rounded-2xl transition-colors duration-500 ${selectedType === option.id ? "bg-[#C9A961] text-black" : "bg-gray-100 text-[#C9A961]"}`}>{option.icon}</div>
                    <div className="space-y-1 text-left">
                      <p className={`font-black uppercase tracking-widest text-xs ${selectedType === option.id ? "text-[#C9A961]" : "text-gray-400"}`}>Selection</p>
                      <h4 className="text-lg font-bold text-black">{option.label}</h4>
                    </div>
                    {selectedType === option.id && <CheckCircle2 className="absolute top-6 right-6 w-6 h-6 text-[#C9A961]" />}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="w-full lg:w-100 space-y-6 lg:sticky lg:top-32">
            {/* Custom Quote Card */}
            {(selectedType === "stairs" || selectedType === "both" || (selectedType === "floor" && floorData.sqft > 0)) ? (
              <div className="bg-[#1A1A1A] p-8 rounded-[2.5rem] shadow-2xl space-y-8 relative overflow-hidden group border border-white/5 animate-in zoom-in-95 duration-500">
                ...
                <div className="space-y-3 pt-4 border-t border-white/5">
                   <div className="flex justify-between items-center text-sm">
                      <span className="text-white/40 font-medium">Project Type</span>
                      <span className="text-white font-black">{selectedType === "stairs" ? "Stairs Only" : selectedType === "floor" ? "Floor Only" : "Floor & Stairs"}</span>
                   </div>
                   ...
                   {(selectedType === "floor" || selectedType === "both") && (
                     <>
                        <div className="flex justify-between items-center text-sm">
                           ...
                           <span className="text-[#C9A961] font-black">{floorData.sqft || 0} SQFT</span>
                        </div>
                        <div className="flex justify-between items-center text-sm">
                           <span className="text-white/40 font-medium">Room Count</span>
                           <span className="text-white font-black">{floorData.roomCount} Rooms</span>
                        </div>
                     </>
                   )}
                   ...
                   {(selectedType === "stairs" || selectedType === "both") && (
                     <>
                        <div className="flex justify-between items-center text-sm">
                           <span className="text-white/40 font-medium">Steps (Straight)</span>
                           <span className="text-white font-black">{stairData.steps}</span>
                        </div>
                        <div className="flex justify-between items-center text-sm">
                           <span className="text-white/40 font-medium">Landings/Box</span>
                           <span className="text-white font-black">{stairData.landings} / {stairData.boxSteps}</span>
                        </div>
                     </>
                   )}
                </div>
              ...
              <div className="bg-white border border-gray-100 p-8 rounded-[2.5rem] shadow-[0_30px_60px_-12px_rgba(0,0,0,0.1)] relative overflow-hidden group">
                ...
                <div className="space-y-8">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-black text-[#C9A961] uppercase tracking-[0.2em]">Estimated Investment</span>
                  </div>

                  <div className="space-y-1">
                    <p className="text-gray-400 text-xs font-bold uppercase tracking-widest">Starting from</p>
                    <div className="flex items-baseline gap-2">
                      <span className="text-6xl font-black text-black tracking-tighter">$2,450</span>
                      <span className="text-[#C9A961] font-bold text-xl uppercase">*</span>
                    </div>
                  </div>
            ...
            <div className="bg-white p-8 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] space-y-6 group border border-gray-100">
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1">
                  <h4 className="text-xl font-black text-black leading-tight">Need a professional <br /> on-site?</h4>
                  <p className="text-sm text-gray-400 font-medium leading-relaxed">We'll visit for free, measure everything, and bring samples.</p>
                </div>
              </div>
              <button 
                onClick={() => setIsModalOpen(true)}
            ...
            <div className="flex items-center justify-center gap-3 py-4 border border-gray-100 rounded-2xl bg-gray-50/50">
              ...
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Licensed in Florida • CRC1331777</span>
            </div>
          </div>
        </div>
      </div>
